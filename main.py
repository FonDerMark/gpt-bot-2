...
from services import data_to_server
...
if DEBUG:
    ...
    async def send_welcome(message: types.Message):
        response = requests.post(f'http://{SERVER_HOST}/api/payment/', data=data_to_server(message, crypto=True)).text
...
async def status(message: types.Message):
    response = requests.post(f'http://{SERVER_HOST}/api/status/', data=data_to_server(message)).text
...
async def request_to_gpt(message: types.Message):
    status_response = requests.post(f'http://{SERVER_HOST}/api/status/', data=data_to_server(message)).text
    response_to_dict = json.loads(status_response)
    extra_messages = response_to_dict['extra_messages']
    day_limit_of_messages = response_to_dict['day_limit_of_messages']
    wait_msg = await message.answer(f'Запрос отправлен, ожидайте ответа🤓\nЗапросов осталось: {day_limit_of_messages}\nЭкстра запросов: {extra_messages}')
    json_response = requests.post(f'http://{SERVER_HOST}/api/request/', data=data_to_server(message)).text
...
async def pre_checkout_query(pre_checkout_q: types.PreCheckoutQuery):
    response = requests.post(f'http://{SERVER_HOST}/api/precheckout/', data=data_to_server(pre_checkout_q))
    if json.loads(response)['precheckout']:
...
async def successful_payment(message: types.Message):
    ...
    response = requests.post(f'http://{SERVER_HOST}/api/payment/', data=data_to_server(message, crypto=True))
    payment_status = json.loads(response)['status']
